...
//	private static final String TAG = "RssListFragment";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		urlFeed = (String) getActivity().getIntent().getExtras().get(LoginFragment.URL);
			...
			imageThread
					.setListener(new ImageDownloader.Listener<ImageView>() {
    ...
    public void customLoadMoreDataFromApi(int offset) {
    	String urlFeed = (String) getActivity().getIntent().getExtras().get(LoginFragment.URL);
	...
	public void setupAdapter() {
		...
		if(listView.getAdapter()==null){
			listViewAdapter=new FeedListViewAdapter(getActivity(),
					android.R.id.list, TumblrModel.getInstance().getListRss(),
					this.imageThread);
	...
	public void onListItemClick(ListView l, View v, int position, long id) {
		FeedVO rssVO = (FeedVO) listView.getAdapter().getItem(position);
		// Start PagerActivity
		Intent i = new Intent(getActivity(), FeedDetailPagerActivity.class);
		i.putExtra(FeedDetailFragment.SELECTED_FEED, rssVO);
		startActivity(i);
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_item_refresh:
			...
			listViewAdapter.setListRss(TumblrModel.getInstance().getListRss());
			listViewAdapter.notifyDataSetChanged();
		default:
